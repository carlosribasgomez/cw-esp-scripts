"""
MODEL FACTORY — Generador maestro para CW ESP
...
def add_standalone_rows(ws, rows):
    """Agrega filas sin invertir (para Info Personal, Giro Positivo)."""
...
def download_photo(model_name, dest_folder):
    """Descarga foto de perfil desde Airtable CW ESP."""
    ...
    if not token:
        print(f"  [SKIP] No AIRTABLE_PAT para descarga de foto")
        return None
    ...
    r = requests.get(
        ...
        params={"filterByFormula": formula, "fields[]": ["Foto de Perfil"]}
    )
    data = r.json()
    records = data.get("records", [])
    if not records:
        print(f"  [SKIP] No hay registro en Airtable para '{model_name}'")
        return None
    
    pics = records[0].get("fields", {}).get("Foto de Perfil", [])
    if not pics:
        print(f"  [SKIP] No hay foto de perfil para '{model_name}'")
        return None
...
class ModelFactory:
    ...
    def generate_merged_xlsx(self):
        ...
        print(f"[XLSX] {output} — {n_sheets} hojas, {n_scripts} scripts")
        return output
    ...
    def generate_guide_html(self):
        ...
        for i, phase in enumerate(phases):
            ...
            copy_all_btn = ('<button class="copy-all" data-copy-all="%s">'
                           'Copiar Todo</button>' % copy_all_data) if all_texts else ''
        ...
        if c.get('nr_waves'):
            ...
            nr_html = (
                ...
                '  <div class="ref-title">\U0001f507 Protocolo NR Waves</div>\n'
                '  <table class="nr-table"><thead><tr>'
                '<th>Wave</th><th>Mensaje</th><th>Nota</th><th></th>'
                '</tr></thead>\n  <tbody>'
                + ''.join(nr_rows)
                + '</tbody></table>\n</div>\n<hr class="divider">\n')
        ...
        if c.get('personal_info'):
            ...
            pi_html = (
                ...
                '  <div class="ref-title">\U0001f464 Info Personal</div>\n'
        ...
        if c.get('positive_spin'):
            ...
            spin_html = (
                ...
                '  <div class="ref-title">\u2728 Giro Positivo</div>\n'
                '  <div class="qi-grid">' + ''.join(items) + '</div>\n'
                '</div>\n<hr class="divider">\n')
        ...
        if c.get('traffic') == 'dating_app' and c.get('meetup_redirect'):
            ...
            mr_html = (
                ...
                '    <span class="phase-name">Redirecci\u00f3n de Cita</span>\n'
        ...
        if c.get('location_handling'):
            ...
            loc_html = (
                ...
                '  <div class="ref-title">\U0001f4cd Manejo de Ubicaci\u00f3n</div>\n'
                '  <div class="qi-grid">' + ''.join(items) + '</div>\n'
                '</div>\n<hr class="divider">\n')
        ...
        info_pairs = [
            ('Nacionalidad', c.get('nationality', '')),
            ('Tipo de P\u00e1gina', c.get('page_type', '')),
            ('Tr\u00e1fico', c.get('traffic', '')),
            ('Trabajo', c.get('job', '')),
            ('Intereses', interests_str),
            ('F\u00edsico', c.get('physical', '')),
            ('Pa\u00edses', c.get('countries', '')),
            ('Idiomas', c.get('languages', '')),
        ]
        profile_grid = ''.join(
        ...
        voice_html = ''
        if c.get('voice'):
            vp = '<strong>\U0001f5e3\ufe0f Voz:</strong> %s' % h(c['voice'])
            if c.get('voice_pet_names'):
                vp += '<br><strong>Apodos:</strong> %s' % h(c['voice_pet_names'])
            if c.get('voice_never'):
                vp += '<br><strong>Nunca decir:</strong> %s' % h(c['voice_never'])
        ...
        if c.get('traffic') == 'dating_app' and c.get('location_handling'):
            ref_toc.append('<a href="#location" class="toc-ref">Ubicaci\u00f3n</a>')
        if c.get('personal_info'):
            ref_toc.append('<a href="#personal-info" class="toc-ref">Personal</a>')
        if c.get('positive_spin'):
            ref_toc.append('<a href="#positive-spin" class="toc-ref">Giro+</a>')
        ref_toc.append(
        ...
        JS = (
            ...
            "ca.textContent='Copiado!';"
            "setTimeout(()=>{ca.classList.remove('ok');ca.textContent='Copiar Todo'},1500)});"
            "return;}});\n"
            ...
            "searchCount.textContent=count+' encontrados'});\n"
        ...
        page = (
            ...
            '<title>%s \u2014 Gu\u00eda del Chatter</title>\n' % h(name_up)
            + CSS + '\n</head>\n<body>\n<div class="page">\n\n'
            '<a href="../" class="back-btn">\u2190 Todas las Creadoras</a>\n\n'
            ...
            'placeholder="Buscar scripts..." autocomplete="off">\n'
            '  <span class="search-count" id="search-count"></span>\n'
            '</div>\n\n'
            + '<div class="info-grid">\n  <div>\n'
            '    <div class="profile">\n'
            '      <h2>\U0001f464 Perfil del Personaje</h2>\n'
        ...
        if c.get('special_notes') or c.get('key_phrases') or c.get('explicit_level'):
            ...
            if c.get('explicit_level'):
                level_map = {'full': ('Expl\u00edcito Completo', '#f85149', '#f8514912', '#f8514933'),
                             'non_explicit': ('No Expl\u00edcito (Lencer\u00eda/Tease)', '#d29922', '#d2992212', '#d2992233'),
                             'soft': ('Suave (Sin Hardcore)', '#e3b341', '#e3b34112', '#e3b34133')}
                ...
                right_parts.append(
                    ...
                    '<strong style="color:%s">Nivel de Contenido:</strong> %s</div>'
                    % (bg, bdr, clr, h(lbl)))
            if c.get('special_notes'):
                right_parts.append(
                    ...
                    'font-size:.84rem;color:var(--yellow)"><strong>Notas:</strong> %s</div>'
                    % h(c['special_notes']))
            if c.get('key_phrases'):
                ...
                right_parts.append(
                    ...
                    'text-transform:uppercase;letter-spacing:.5px">Frases Clave:</strong><div style="'
        ...
        page += (
            ...
            'Manejo de Objeciones y Resistencias</div>\n'
            '    <div style="font-size:.78rem;color:var(--muted)">'
            'Secuencias completas para Infloww</div>\n'
            '  </div>\n'
            ...
            'CW ESP \u00b7 Script Manager \u00b7 2026</div>\n\n'
    ...
    def generate_objections_html(self):
        ...
        categories = {
            "obj": {"label": "A. Manejo de Objeciones", "emoji": "\U0001f4b0",
                    "cls": "cat-obj", "protos": []},
            "res": {"label": "B. Manejo de Resistencias", "emoji": "\U0001f500",
                    "cls": "cat-res", "protos": []},
            "sit": {"label": "C. Scripts Situacionales", "emoji": "\u26a1",
                    "cls": "cat-sit", "protos": []},
        }
        ...
        for ck in ("obj", "res", "sit"):
            ...
            for base_name, proto in cat["protos"]:
                ...
                proto_parts.append(
                    ...
                    '<span class="p-meta">%d pasos \u00b7 %d seq</span>'
        ...
        page = (
            ...
            '<meta charset="UTF-8">\n'
            '<meta name="viewport" content="width=device-width, initial-scale=1.0">\n'
            '<title>%s \u2014 Manejo de Objeciones y Resistencias</title>\n' % h(name_up)
            + CSS + '\n</head>\n<body>\n<div class="page">\n\n'
            '<a href="./" class="back-btn">\u2190 Gu\u00eda de %s</a>\n\n' % h(name)
            + '<div class="hero">\n'
            '  <img src="%s" alt="%s" class="hero-photo">\n'
            '  <div>\n    <h1>%s \u2014 Objeciones y Resistencias</h1>\n'
            '    <div class="tagline">Secuencias completas para Infloww</div>\n'
            '  </div>\n</div>\n\n'
            % (photo, h(name), h(name_up))
            + '<div class="info-bar">\n'
            '  <span class="info-pill"><b>%d</b> hojas</span>\n'
            '  <span class="info-pill"><b>%d</b> scripts</span>\n'
            '  <span class="info-pill"><b>%d</b> protocolos</span>\n'
            '</div>\n\n' % (total_sheets, total_scripts, total_protos)
            + '<nav class="toc" id="toc">\n  '
            + '\n  '.join(toc_items) + '\n</nav>\n\n'
            + '\n'.join(proto_parts) + '\n\n'
            + '<div style="text-align:center;padding:24px 0 10px;color:var(--muted);font-size:.73rem">'
            'CW ESP \u00b7 Script Manager \u00b7 2026<br>'
            '%s \u2014 OBJ/RES/SIT</div>\n\n' % h(name_up)
            + '</div>\n<button class="top-btn" id="top-btn">\u2191</button>\n\n'
            + JS + '\n</body>\n</html>')
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write(page)
        print("[HTML] Objeciones: %s" % path)
        return path

    def generate_all(self):
        print(f"\n{'='*60}")
        print(f"GENERANDO: {self.c['name']}")
        print(f"{'='*60}")
        ...
        print(f"[HECHO] {self.c['name']} completo!")
        return xlsx_path


if __name__ == "__main__":
    print("ModelFactory listo. Importar y usar con configs de modelos.")
    print("Ejemplo: from model_factory import ModelFactory")
